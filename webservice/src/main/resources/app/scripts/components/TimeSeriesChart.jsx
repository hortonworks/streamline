...
export default class TimeSeriesChart extends Component {
    ...
    drawChart(){
        ...
        this.svg
            .attr("viewBox", "-46 -6 " + (this.width+82) + " " + (this.height+27) )
...
TimeSeriesChart.defaultProps = {
    ...
    getYScale() {
        return d3.scale.linear().range([this.height-1, 0]);
    },
    getXAxis() {
    ...
    getSvgArea(){
        const self = this;
        return d3.svg.area()
            .x(function(d) { return self.x(d[self.props.xAttr]); })
            .y0(this.height)
    ...
    setXDomain(){
        ...
        var dateRangeAfterDiff = [d3.time.minute.offset(dateRange[0], -(diff/2)), d3.time.minute.offset(dateRange[1], +(diff/2))];
    ...
    drawXAxis () {
        ...
        container['xAxisEl'] = container.insert("g",":first-child")
            .attr("class", "x axis")
    ...
    drawYAxis(x) {
        ...
        this.yAxisGrp = this.container.insert("g",":first-child")
            .attr("class", "y axis");
        this.yAxisGrp.ticks = this.yAxisGrp.call(yAxis);
        this.yAxisGrp.append('text')
            .text(this.yAttr).attr("text-anchor", "end")
            .attr("y", 6)
            .attr("dy", ".75em")
    ...
    showTooltip(data){
        ...
        if(clientLeft < 0 || clientLeft > this.width) return;
