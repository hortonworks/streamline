...
export default class PieChart extends Component {
  ...
  setArc() {
    this.arc = d3.svg.arc().outerRadius(this.radius - this.props.outerRadius).innerRadius(this.radius - this.props.innerRadius).startAngle(function(d) {
      return isNaN(d.startAngle)
        ? 0
        : d.startAngle;
    }).endAngle(function(d) {
      return isNaN(d.endAngle)
        ? 0
        : d.endAngle;
    });
  }
  ...
  drawPie() {
    ...
    this.arc_g = this.container.selectAll(".arc").data(this.pie);

    // .data(this.pie(this.data))
    this.arc_g.exit().remove();

    this.arc_g.enter().append("g").style("fill", function(d) {
    ...
    this.paths.enter().append("path").each(function(d) {
      this._current = d;
    }).attr("d", this.arc);
    ...
    if (this.props.showLabels) {
      ...
      this.texts.enter().append("text").attr("transform", function(d) {
        return "translate(" + self.arc.centroid(d) + ")";
      }).attr("dy", ".35em").style('fill', 'black').style('text-anchor', 'middle').text(function(d) {
        return d.data.name;
      });
    }
    ...
    this.titles.exit().remove();

    this.titles.enter().append("title");
  ...
  transition() {
    ...
    this.paths.transition().duration(500).attrTween("d", arcTween);

    if (this.props.showLabels) {
      this.texts.data(function(d, i) {
        return [self.pie(self.props.data)[i]];
      });
      this.texts.transition().duration(200).delay(500).attr("transform", function(d) {
        return "translate(" + self.arc.centroid(d) + ")";
      });
    }

    this.titles.text(function(d) {
