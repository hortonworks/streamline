...
export default class EditDefinition extends Component {
  ...
  customData = () => {
    ...
    let tempArr = [];
    let mvn = {
      mavenDeps: obj.mavenDeps
    };
    let fields = {
      topologyComponentUISpecification: obj.topologyComponentUISpecification
    };
    delete obj.id;
          ...
          if (x.topologyComponentUISpecification) {
            return <div className="form-group" key={i}>
              ...
              <div className="col-sm-12">
                <ReactCodemirror ref="JSONCodemirror" value={JSON.stringify(x.topologyComponentUISpecification, "", 2)} options={jsonoptions}/>
              </div>
            </div>;
          } else {
            return <div className="form-group" key={i}>
              <label className="col-sm-12 control-label">{_.keys(x)[0]}</label>
              <div className="col-sm-12">
                <input ref={_.keys(x)[0]} disabled={this.props.viewMode
                  ? true
                  : _.keys(x)[0] === "mavenDeps"
                    ? false
                    : true} type="text" className="form-control" value={(x[_.keys(x)[0]]) || ''} onChange={this.handleChange}/>
              </div>
            </div>;
          }
        })
