...
export default class NormalizationNodeForm extends Component {
    ...
    Promise.all(edgePromiseArr).then(edgeResults => {
      ...
      Promise.all(promiseArr).then((results) => {
        ...
        streams.map((s) => {
          ...
          transformers.map((o) => {
            arr.push({streamId: s, inputField: o.inputField, outputField: o.outputField});
            if (!_.find(outputSchemaArr, {name: o.outputField.name})) {
              outputSchemaArr.push(o.outputField);
              mappingsArr.push({value: o.outputField.name, label: o.outputField.name});
            }
          });
        });
  ...
  saveStream() {
    ...
    let passStreamData = {
      streamId: 'normalization_stream_' + this.nodeData.id,
      fields: []
    };
    TopologyREST.createNode(topologyId, versionId, 'streams', {body: JSON.stringify(passStreamData)}).then(result => {
      ...
      TopologyREST.updateNode(topologyId, versionId, nodeType, self.nodeData.id, {
        body: JSON.stringify(self.nodeData)
      }).then((node) => {
        self.nodeData = node;
  ...
  getInputStreams(inputStreams) {
    let arr = [];
    inputStreams.map((s) => {
      arr.push({value: s.streamId, label: s.streamId});
    });
    this.setState({inputStreamsArr: arr, currentStream: arr[0].value});
  }

  getInputSchemaFields(id) {
    let streamData = _.find(this.state.inputStreams, {
      streamId: id
        ? id
  ...
  getHeader() {
    let iconClass = this.state.showOptionalFields
      ? "fa fa-caret-down"
      : "fa fa-caret-right";
    return (
      <div>
  ...
  handleStreamChange(obj) {
    ...
    this.setState({currentStream: obj.value});
  ...
  addAllFields = (e) => {
    ...
    this.setState({
      outputSchemaFields: [
        ...this.state.outputSchemaFields,
        ...outputSchemaArr
      ],
      outputFieldsMappingArr: [
        ...this.state.outputFieldsMappingArr,
        ...mappingsArr
      ],
      transformers: [
  ...
  addFieldToOutputSchema = (e) => {
    ...
    if (_.find(this.state.outputSchemaFields, {name: name})) {
      ...
      if (!_.find(this.state.outputFieldsMappingArr, {value: optionObj.value})) {
        ...
        optionsArr = [...this.state.outputFieldsMappingArr];
      }
  ...
  mapInputField = () => {
    this.setState({showMappingFields: true});
  }
  ...
  deleteOutputField(name) {
    let outputField = _.find(this.state.outputSchemaFields, {name: name}),
      ...
      mappingsArr = [];
    let flag = tArr.filter(t => {
      return t.streamId === this.state.currentStream;
    }).length === this.state.inputSchemaFields.length;
    arr.map((m) => {
      mappingsArr.push({value: m.name, label: m.name});
    });
    this.setState({
      outputSchemaFields: arr,
      outputFieldsMappingArr: mappingsArr,
  ...
  handleMappingChange(f, obj) {
    ...
    if (t !== -1) {
      ...
      transformers.push({inputField: inputField, outputField: outputField, streamId: this.state.currentStream});
    }
  ...
  handleFieldNameChange(e) {
    ...
    if (this.validateName(name, index)) {
      ...
      outputFieldsMappingArr[index] = {
        value: name,
        label: name
      };
      this.setState({outputSchemaFields: outputSchemaFields, transformers: transformers, outputFieldsMappingArr: outputFieldsMappingArr});
    }
  }
  ...
  handleEditableReject(editable) {
    ...
    if (this.editableFieldIndex !== undefined && this.editableFieldIndex !== '') {
      ...
      outputFieldsMappingArr[this.editableFieldIndex] = {
        value: this.editableFieldName,
        label: this.editableFieldName
      };
      this.setState({outputSchemaFields: outputSchemaFields, transformers: transformers, outputFieldsMappingArr: outputFieldsMappingArr});
    }
    this.editableFieldIndex = '';
  ...
  getData() {
    ...
    this.state.inputStreams.map((s) => {
      let streamFields = this.state.transformers.filter((o) => {
        return o.streamId === s.streamId;
      });
      if (streamFields.length) {
        ...
        streamFields.map((f) => {
          obj.transformers.push({inputField: f.inputField, outputField: f.outputField, converterScript: null});
        });
  ...
  handleSave(name, description) {
    ...
    return TopologyREST.getNode(topologyId, versionId, nodeType, nodeId).then(result => {
      let newData = result;
      newData.config.properties = data;
      newData.name = name;
      newData.description = description;
      let outputStreamData = {
        streamId: newData.outputStreams[0].streamId,
        fields: this.state.outputSchemaFields
      };
      return TopologyREST.updateNode(topologyId, versionId, 'streams', newData.outputStreams[0].id, {body: JSON.stringify(outputStreamData)}).then((stream) => {
    ...
    return (
      <div>
        <Tabs id="NormalizationForm" defaultActiveKey={1} className="schema-tabs">
            ...
            <form className="form-horizontal">
              <div className="form-group">
                ...
                <div className="col-sm-6">
                  <Select value={this.state.currentStream} options={this.state.inputStreamsArr} onChange={this.handleStreamChange.bind(this)} required={true} searchable={false} clearable={false}/>
                </div>
              </div>
            </form>
            <div className="row">
                  ...
                  <div className="box-head">
                    ...
                    {editMode
                      ? <div className="box-controls">
                          <a href="javascript:void(0);" title="Add all fields" data-rel="tooltip" className="select-list" onClick={this.addAllFields}>
                            <i id="selectAll" className={this.state.enableAddAll
                              ? "fa fa-square-o"
                              : "fa fa-check-square-o"}></i>
                          </a>
                        </div>
                      : null}
                  </div>
                    ...
                    <ul className="field-list input-field-list">
                      {this.state.inputSchemaFields.length === 0
                        ...
                        : this.state.inputSchemaFields.map((f, i) => {
                          let t = _.find(this.state.transformers, {
                            streamId: this.state.currentStream,
                            inputField: f
                          });
                          return (
                            <li key={i} className={f.isOptional
                              ? "optional"
                              : "mandatory"}>
                              {editMode
                                ? <OverlayTrigger trigger="click" placement="bottom" overlay={< Popover id = "popover-trigger-click" > <button type="button" className="btn btn-primary btn-sm" onClick={this.mapInputField} data-id={f.name}>Map To Existing</button>
                                    {'\n'} <button type = "button" className = "btn btn-primary btn-sm" onClick = {
                                    this.addFieldToOutputSchema
                                  }
                                  data-id = {
                                    f.name
                                  }> Add New </button> <hr/ > {
                                    this.state.showMappingFields
                                      ? <Select value={t
                                          ? t.outputField.name
                                          : ''} options={this.state.outputFieldsMappingArr} onChange={this.handleMappingChange.bind(this, f.name)} searchable={false} disabled={!this.state.editMode} clearable={false}/>
                                      : null
                                  } < /Popover>} onClick={() => {
                                    this.setState({showMappingFields: false});
                                  }}>
                              ...
                              {t !== undefined
                                ? (
                                  <span className="label label-danger pull-right">{t.outputField.name}</span>
                                )
                                : null}
                            </li>
                          );
                      ...
                      {this.state.outputSchemaFields.map((f, i) => {
                        return (
                          <li key={i} className={f.isOptional
                            ...
                            : "mandatory"}>
                            {editMode
                              ? <Editable ref={"fieldNameEditable" + i} inline={true} resolve={this.saveFieldName.bind(this)} reject={this.handleEditableReject.bind(this)}>
          ...
          <Tab eventKey={2} title="Output Streams" unmountOnExit={true}></Tab>
        </Tabs>
      </div>
    );
  }
