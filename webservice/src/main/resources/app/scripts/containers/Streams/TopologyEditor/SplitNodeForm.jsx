  ...
  fetchData() {
    ...
    Promise.all(promiseArr).then((results) => {
      ...
      results[0].entities.map((entity) => {
        arr.push({id: entity.id, value: entity.id, label: entity.name});
      });
            ...
            <form className="form-horizontal">
              <div className="form-group">
                ...
                <div className="col-sm-6">
                  <Select value={this.state.fileId} options={this.state.fileArr} onChange={this.handleFileChange.bind(this)} required={true} disabled={!this.state.editMode}/>
                </div>
              </div>
              <div className="form-group">
                <label className="col-sm-3 control-label">Splitter Class*</label>
                <div className="col-sm-6">
                  <input name="splitterClassName" value={this.state.splitterClassName} onChange={this.handleValueChange.bind(this)} type="text" className={editMode && showError && changedFields.indexOf("splitterClassName") !== -1 && this.state.splitterClassName === ''
                    ? "form-control invalidInput"
                    : "form-control"} required={true} disabled={!this.state.editMode}/>
                </div>
              </div>
              {/*<div className="form-group">
