...
export default class MetricsContainer extends Component {
  ...
  fetchData(loadingKeyArr) {
    ...
    if (selectedComponentId !== '') {
      ...
      MetricsREST.getComponentStatsMetrics(topologyId, selectedComponentId, startDate.toDate().getTime(), endDate.toDate().getTime()).then((res) => {
      ...
      MetricsREST.getComponentLatencyMetrics(topologyId, selectedComponentId, startDate.toDate().getTime(), endDate.toDate().getTime()).then((res) => {
        if (res.responseMessage !== undefined) {
          FSReactToastr.error(
            <CommonNotification flag="error" content={res.responseMessage}/>, '', toastOpt);
          ...
          this.setState({loadingLatencyQueue: false, latencyData: latencyData});
        }
      });
    } else {
      MetricsREST.getTopologyMetrics(topologyId, startDate.toDate().getTime(), endDate.toDate().getTime()).then((res) => {
        if (res.responseMessage !== undefined) {
          FSReactToastr.error(
            <CommonNotification flag="error" content={res.responseMessage}/>, '', toastOpt);
  ...
  getGraph(name, data) {
    const self = this;
    return <TimeSeriesChart color={d3.scale.category20c().range(['orange', 'red'])} ref={name} data={data} height={this.state.graphHeight} setXDomain={function() {
      ...
      if (!this.brush.empty()) {
        ...
        self.setState({
          startDate: moment(newChartXRange[0]),
  ...
  handleFilterChange = (e) => {
    if (e) {
      this.setState({
        selectedComponentId: e.nodeId,
        selectedComponentName: e.uiname
      }, () => {
        this.fetchData(["Record", "LatencyQueue"]);
      });
    } else {
        ...
        <div className="form-horizontal">
          <div className="form-group">
            ...
            <div className="col-sm-2">
              <Select value={selectedComponentId} onChange={this.handleFilterChange} options={this.props.components} placeholder="Select Component" clearable={true} valueKey="nodeId" labelKey="uiname"/>
            </div>
            <label className="col-sm-2 col-sm-offset-2 control-label">Date/Time Range Filter:</label>
            <div className="col-sm-4">
              <DatetimeRangePicker timePicker timePicker24Hour showDropdowns timePickerSeconds locale={locale} startDate={this.state.startDate} endDate={this.state.endDate} ranges={this.state.ranges} onApply={this.handleEvent} opens="left">
            ...
            <div className="row col-md-4" style={{
              ...
              <div style={{
                ...
                {this.state.loadingRecord
                  ? loader
                  : this.getGraph('inputOutput', inputOutputData)}
              </div>
            </div>
            <div className="row col-md-4" style={{
              'textAlign': 'center'
              ...
              <div style={{
                ...
                {this.state.loadingRecord
                  ? loader
                  : this.getGraph('ackedTuples', ackedData)}
              </div>
            </div>
            <div className="row col-md-4" style={{
              'textAlign': 'center'
              ...
              <div style={{
                ...
                {this.state.loadingRecord
                  ? loader
                  : this.getGraph('FailedTuples', failedData)}
              </div>
            </div>
          </Panel>
          <Panel header={selectedComponentName.length > 0
            ...
            <div className="row col-md-6" style={{
              ...
              <h5>{selectedComponentName.length > 0
                  ? 'Latency'
                  : 'Processed Time'}</h5>
              <div style={{
                height: '150px',
                ...
                {this.state.loadingLatencyQueue
                  ? loader
                  : (selectedComponentName.length > 0
                    ? this.getGraph('Latency', latencyData)
                    : this.getGraph('Latency', processTimeData))}
              </div>
            </div>
            <div className="row col-md-6" style={{
              'textAlign': 'center'
