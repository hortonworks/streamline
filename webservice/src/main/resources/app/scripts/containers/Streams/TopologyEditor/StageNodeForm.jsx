...
export default class StageFormNode extends Component {
  ...
  constructor(props) {
    ...
    let parallelismObj = config.filter((f) => {
      return f.name === 'parallelism';
    });
  ...
  fetchFieldsFromSource() {
    ...
    Promise.all(promiseArr).then(results => {
      ...
      Promise.all(streamPromiseArr).then(streamResults => {
    ...
    return (
      <div>
        <Tabs id="stageForm" defaultActiveKey={1} className="schema-tabs">
            ...
            <form className="form-horizontal">
              <div className="form-group">
                ...
                <div className="col-sm-6">
                  <Select value={transform} options={transformTypesArr} onChange={this.handleTransformChange.bind(this)} required={true} searchable={false} disabled={!editMode}/>
                </div>
              </div>
              <div className="form-group">
                <label className="col-sm-3 control-label">Fields*</label>
                <div className="col-sm-6">
                  <Select value={transformFields} options={transformFieldsArr} onChange={this.handleFields.bind(this)} required={true} multi={true} clearable={false} joinValues={true} disabled={!editMode}/>
                </div>
              </div>
              {transform === 'enrichment'
                ? [ < div key = "1" className = "form-group" > <label className="col-sm-3 control-label">Entry Expiration Interval *</label> < div className = "col-sm-6" > <input name="entryExpirationInterval" value={entryExpirationInterval} onChange={this.handleValueChange.bind(this)} type="number" className={editMode && showError && changedFields.indexOf("entryExpirationInterval") !== -1 && entryExpirationInterval === ''
                  ? "form-control invalidInput"
                  : "form-control"} disabled={!editMode} min="0" inputMode="numeric"/> < /div>
                                                                </div >, < div key = "2" className = "form-group" > <label className="col-sm-3 control-label">Entry Refresh Interval *</label> < div className = "col-sm-6" > <input name="entryRefreshInterval" value={entryRefreshInterval} onChange={this.handleValueChange.bind(this)} type="number" className={editMode && showError && changedFields.indexOf("entryRefreshInterval") !== -1 && entryRefreshInterval === ''
                  ? "form-control invalidInput"
                  : "form-control"} disabled={!editMode} min="0" inputMode="numeric"/> < /div>
                                                                </div >, < div key = "3" className = "form-group" > <label className="col-sm-3 control-label">Max Cache Size *</label> < div className = "col-sm-6" > <input name="maxCacheSize" value={maxCacheSize} onChange={this.handleValueChange.bind(this)} type="number" className={editMode && showError && changedFields.indexOf("maxCacheSize") !== -1 && maxCacheSize === ''
                  ? "form-control invalidInput"
                  : "form-control"} disabled={!editMode} min="0" inputMode="numeric"/> < /div>
                                                                </div >
                ]
          ...
          <Tab eventKey={2} title="Output Streams"></Tab>
        </Tabs>
      </div>
    );
  }
