...
class ModelRegistryContainer extends Component {
  ...
  fetchData = () => {
    ModelRegistryREST.getAllModelRegistry().then((models) => {
      if (models.responseMessage !== undefined) {
        this.setState({fetchLoader: false});
        FSReactToastr.error(
          <CommonNotification flag="error" content={models.responseMessage}/>, '', toastOpt);
      } else {
  ...
  handleSave = () => {
    if (this.refs.addModelRegistry.validate()) {
      this.refs.addModelRegistry.handleSave().then((model) => {
        ...
        if (model.responseMessage !== undefined) {
          ...
          this.setState({
            ...
            FSReactToastr.success(
              <strong>Model added successfully</strong>
            );
          });
        }
      });
  ...
  handleDelete(id) {
    ...
    BaseContainer.refs.Confirm.show({title: 'Are you sure you want to delete this model?'}).then((confirmBox) => {
      ModelRegistryREST.deleteModelRegistry(id).then((model) => {
        this.fetchData();
        confirmBox.cancel();
        if (model.responseMessage !== undefined) {
          FSReactToastr.error(
            <CommonNotification flag="error" content={model.responseMessage}/>, '', toastOpt);
        } else {
          FSReactToastr.success(
            <strong>Model deleted successfully</strong>
          );
        }
      });
    }, (Modal) => {});
  ...
  handleEdit(id) {
    ...
    ModelRegistryREST.getModelRegistry(id).then((model) => {
      if (model.responseMessage !== undefined) {
        FSReactToastr.error(
          <CommonNotification flag="error" content={model.responseMessage}/>, '', toastOpt);
      } else {
