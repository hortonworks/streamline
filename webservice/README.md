...
*Sample Input*
    ...
    curl -X POST -v -F parserJar=@parsers-0.1.0.jar -F parserInfo=@ParserInfo.json http://localhost:8080/api/v1/catalog/parsers
...
```json    
{
  ...
  "entity": {
    ...
    "name": "TestParser",
    "className": "org.apache.streamline.registries.parser.json.JsonParser",
    "jarStoragePath": "/tmp/parsers-0.1.0.jar",
    "parserSchema": {
      "fields": [
        {
          "name": "DeviceName",
...
```json
{
  ...
  "entities": [
    ...
    },
    {
      ...
      "jarStoragePath": "/tmp/test-0.1.0.jar",
      "parserSchema": {
        "fields": [{"name": "DeviceName","type": "STRING"}]
      },
      "model": 1,
...
GET /api/v1/catalog/parsers/verify-upload
    ...
    curl -X POST -i -F parserJar=@parsers-0.1.0.jar http://localhost:8080/api/v1/catalog/parsers/upload-verify
