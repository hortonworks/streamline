...
#Following steps deal with starting up Streamline server locally and running a topology
cd /root/streamline
#Copy the 3 files core-site.xml, hdfs-site.xml and hbase-site.xml to the directory inside Streamline which is storm/src/main/resources. Note that this step is not needed to setup Streamline server itself. However, to run a sample Streamline topology that interacts with hbase and hdfs we need to package these files in the topology jar. The topology jar will be created when we compile Streamline at top level. The commands for copying these files are as below.
cp /etc/hadoop/conf/core-site.xml /root/streamline/storm/src/main/resources/
cp /etc/hadoop/conf/hdfs-site.xml /root/streamline/storm/src/main/resources/
cp /etc/hbase/conf/hbase-site.xml /root/streamline/storm/src/main/resources/
mvn clean install -DskipTests=true
cp /root/streamline/storm/target/storm-0.1.0-SNAPSHOT.jar /tmp/
#Above step is to copy the storm jar that contains all components for running the topology to a location expected by Streamline. Default location is /tmp. To change that, update the value for streamlineStormJar in /root/streamline/webservice/conf/streamline.yaml and copy the storm jar to that location
#In /root/streamline/webservice/conf/streamline.yaml check the fileStorageConfiguration section. Update the fsUrl to the correct value from ambari hdfs configuration. By default the value for fsUrl is “hdfs://localhost:9000”. Sandbox ambari hdfs configuration by default uses the port 8020. Hence, the new value should be "hdfs://sandbox.hortonworks.com:8020" where sandbox.hortonworks.com is the hostname of the sandbox. Below is  the command to replace the value
sed -i 's/hdfs:\/\/localhost:9000/hdfs:\/\/sandbox.hortonworks.com:8020/g' /root/streamline/webservice/conf/streamline.yaml
#Under the same configuration check the value for key directory and create hdfs directory with the same value. The is the directory that will be used by Streamline for storing different jars in hdfs. You can use the default value of /tmp/test-hdfs or use a different value. The command for creating hdfs directory for default value are as below.
hdfs dfs -mkdir /tmp/test-hdfs
#Since the default port used by Streamline to start the web server will conflict with ambari server port, execute the below command to update ports 8080 and 8081 to available ports. In this case available ports are 21000 and 21001. Also note that if you pick other ports than the ones mentioned here you might need to change port forwarding settings to be able to reach the Streamline web server on sandbox from browser on the host running the sandbox.
sed -i 's/8080/21000/' /root/streamline/webservice/conf/streamline.yaml
sed -i 's/8081/21001/' /root/streamline/webservice/conf/streamline.yaml
#Now start the Streamline web server by executing below commands
cd /root/streamline/webservice/
...
#Change the hdfs url.
sed -i 's/hdfs:\/\/localhost:9000/hdfs:\/\/sandbox.hortonworks.com:8020/g' /root/streamline/bin/topology
#Change the port for Streamline used by curl script load-device.sh
sed -i 's/8080/21000/' /root/streamline/bin/load-device.sh
cd /root/streamline/bin/
./load-device.sh 
...
java -cp /root/streamline/simulator/target/simulator-0.1.0-SNAPSHOT.jar org.apache.streamline.simulator.CLI -b sandbox.hortonworks.com:6667 -t nest-topic -f /root/streamline/simulator/src/main/resources/nest-streamline-messages
